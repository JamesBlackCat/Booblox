...
local function scanKillParts()
    ...
    for _, desc in ipairs(sections:GetDescendants()) do
        if desc:IsA("BasePart") and desc.Material == Enum.Material.Neon then
            ...
            if namePass or folderPass then
...
floodBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(10, function()
        ...
        local lastDmg     = tick()
        local floodElapsed = 0  -- for time-based acceleration
        ...
        floodConn = RunService.Heartbeat:Connect(function(dt)
            ...
            -- Stop rising when within 60 studs of player (keeps pressure consistent)
            if dist <= 60 then
                -- Do nothing — let flood stay close but stop accelerating
            else
                -- Time-based acceleration: every 30s speed doubles, capped at 5x
                local timeMultiplier = math.min(5, 1 + math.floor(floodElapsed / 30))
                local curSpeed = speed * timeMultiplier
                -- Distance catch-up multiplier on top
                if dist > 120 then
                    curSpeed = curSpeed * 3
                elseif dist > 80 then
                    curSpeed = curSpeed * 1.8
                end
...
mrEvilBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(10, function()
        ...
        body.Position    = h.Position + Vector3.new(0, 0, 30)  -- spawn behind player (Z axis), never inside a tower wall
        ...
        billboard.Adornee       = body          -- explicit adornee — required on some executors
        ...
        -- Fallback red background so billboard is always visible even if image fails
        ...
        -- ── Smoke-demon aura particles ──────────────────────────
        -- Each property set in its own pcall so a bad value never stops execution
        ...
        pcall(function() particles.Rate           = 240 end)
        pcall(function() particles.Rotation       = NumberRange.new(0, 360) end)
        pcall(function() particles.RotSpeed       = NumberRange.new(-120, 120) end)
        pcall(function() particles.Speed          = NumberRange.new(3, 8) end)
        pcall(function() particles.SpreadAngle    = Vector2.new(180, 180) end)  -- MAX is 180, not 360 — 360 crashes the callback
        pcall(function() particles.Lifetime       = NumberRange.new(0.8, 1.8) end)
        pcall(function()
