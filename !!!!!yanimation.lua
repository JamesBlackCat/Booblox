...
function applyAnim(slot, id)
    ...
    -- Step 1: write the new ID into every Animation object for this slot
    local applied = false
    ...
    -- Step 2: stop ALL playing tracks so the Animate script must reload them.
    -- Roblox's Animate LocalScript pre-loads tracks by ID; if we only change
    -- the Animation object's ID the existing loaded track still plays the old
    -- clip. Stopping all tracks forces Animate to re-load with the new IDs.
    if Hum then
    ...
    -- Step 3: bounce the Animate LocalScript so it re-initialises its track
    -- table and immediately picks up the updated AnimationId values.
    pcall(function()
...
-- MAIN WINDOW
-- =====================================================================
-- 87% of the screen, centered — works on iOS, Android, and PC
local Win = mkFrame(GUI,
    UDim2.new(0.87, 0, 0.87, 0),
    UDim2.new(0.065, 0, 0.065, 0),
    T.Bg, "MainWin")
Win.Visible = false
Win.ZIndex  = 10
mkCorner(Win, 14)
