...
local Packs        = {}
local Emotes         = {}
local FloatingEmotes = {}  -- emote index -> floating TextButton Frame
local UndoStack    = {}
local RedoStack    = {}
...
-- NormalizeAnimationId — accepts any common format, returns "rbxassetid://XXXXXXX" or nil
local function NormalizeAnimationId(input)
    if not input then return nil end
    local s = tostring(input):match("^%s*(.-)%s*$")
    if s == "" then return nil end
    -- Already a full rbxassetid URL
    if s:match("^rbxassetid://%d+$") then return s end
    -- Extract numeric id from any format
    local id = extractId(s)
    if id then return "rbxassetid://" .. id end
    warn("[AnimStudio] NormalizeAnimationId: could not parse '" .. tostring(input) .. "'")
    return nil
end
...
function AC:Init(char)
    ...
    -- Use WaitForChild so we don't grab nil if the Animator isn't parented yet.
    -- Falls back to FindFirstChildOfClass and finally creates one if still absent.
    self.Animator = self.Hum:FindFirstChildOfClass("Animator")
        or self.Hum:WaitForChild("Animator", 3)
    if not self.Animator then
        warn("[AnimStudio] Animator missing after 3s wait — creating one")
        local a = Instance.new("Animator")
        a.Parent = self.Hum
        self.Animator = a
    end
    if not self.Animator then
        warn("[AnimStudio] Could not obtain Animator — aborting Init")
        return
    end
...
-- NOTE: CharacterRemoving is handled once in the CLEANUP section below,
-- which also tears down the Animation Editor. Do NOT add a second connection here.
...
local EmSearch   = mkBox(EmTop,"Search emotes…",UDim2.new(0.42,-2,1,-4),UDim2.new(0,0,0,2)); EmSearch.TextSize=11
local FavToggle  = mkBtn(EmTop,"★ Favs", UDim2.new(0.15,-2,1,-4),UDim2.new(0.43,2,0,2),T.Card); FavToggle.TextSize=10
local FloatFilter= mkBtn(EmTop,"⊞ Float",UDim2.new(0.16,-2,1,-4),UDim2.new(0.59,2,0,2),T.Card); FloatFilter.TextSize=10
local AddEmBtn   = mkBtn(EmTop,"+ Add",  UDim2.new(0.22,-2,1,-4),UDim2.new(0.76,2,0,2),T.Accent); AddEmBtn.TextSize=10

local showFavs  = false
local showFloat = false

-- Forward-declare as a local so ALL closures below capture the same upvalue.
-- The actual body is assigned further down after all helpers are defined.
local refreshEmotes

FavToggle.MouseButton1Click:Connect(function()
    showFavs = not showFavs; showFloat = false
    FavToggle.BackgroundColor3  = showFavs  and T.Warn or T.Card
    FloatFilter.BackgroundColor3= T.Card
    FavToggle.Text  = showFavs  and "★ All"  or "★ Favs"
    FloatFilter.Text= "⊞ Float"
    if refreshEmotes then refreshEmotes() end
end)
FloatFilter.MouseButton1Click:Connect(function()
    showFloat = not showFloat; showFavs = false
    FloatFilter.BackgroundColor3= showFloat and T.Info or T.Card
    FavToggle.BackgroundColor3  = T.Card
    FloatFilter.Text= showFloat and "⊞ All"  or "⊞ Float"
    FavToggle.Text  = "★ Favs"
    if refreshEmotes then refreshEmotes() end
end)

local EmScroll = mkScroll(EP,UDim2.new(1,0,1,-42),UDim2.new(0,0,0,40))
mkGrid(EmScroll,UDim2.new(0,106,0,130),UDim2.new(0,6,0,6)); mkPad(EmScroll,4,4,4,4)

-- ── Context menu helpers ──────────────────────────────────────────────
local activeContextMenu = nil
local function closeContextMenu()
    if activeContextMenu then
        pcall(function() activeContextMenu:Destroy() end)
        activeContextMenu = nil
    end
end

-- Close the context menu if the user clicks anywhere outside it
UserInputService.InputBegan:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1
    or inp.UserInputType == Enum.UserInputType.Touch then
        -- Small delay so the dots button's own click fires first
        task.delay(0.04, function()
            -- Only close if the menu still exists (not already closed by a menu-item click)
...
-- ── Rename dialog ─────────────────────────────────────────────────────
local function showRenameDialog(em, onDone)
    local ov = mkFrame(GUI,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),Color3.new(0,0,0),"ROv")
    ov.BackgroundTransparency=0.45; ov.ZIndex=80
    local dlg = mkFrame(GUI,UDim2.new(0,320,0,152),UDim2.new(0.5,-160,0.5,-76),T.Surface,"RDlg")
    dlg.ZIndex=81; mkCorner(dlg,12); mkStroke(dlg,T.Border,1); mkPad(dlg,14,14,14,14)
    mkLabel(dlg,"Rename Emote",UDim2.new(1,0,0,22),UDim2.new(0,0,0,0),T.Accent,14).Font=Enum.Font.GothamBold
    local nb = mkBox(dlg,"New name…",UDim2.new(1,0,0,30),UDim2.new(0,0,0,28))
    nb.Text = em.name; nb.TextSize = 12
    local function cl() pcall(function() ov:Destroy(); dlg:Destroy() end) end
    local sv = mkBtn(dlg,"Rename",UDim2.new(0.48,-2,0,28),UDim2.new(0,0,1,-28),T.OK); sv.TextSize=11
    local ca = mkBtn(dlg,"Cancel",UDim2.new(0.48,-2,0,28),UDim2.new(0.52,2,1,-28),T.Bad); ca.TextSize=11
    ca.MouseButton1Click:Connect(cl)
    sv.MouseButton1Click:Connect(function()
        local n = nb.Text ~= "" and nb.Text or em.name
        em.name = n; cl()
        if onDone then onDone() end
        notify("Renamed to: "..n,"success",2)
    end)
end

-- ── Decal ID dialog ───────────────────────────────────────────────────
local function showDecalDialog(em, onDone)
    local ov = mkFrame(GUI,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),Color3.new(0,0,0),"DOv")
    ov.BackgroundTransparency=0.45; ov.ZIndex=80
    local dlg = mkFrame(GUI,UDim2.new(0,340,0,168),UDim2.new(0.5,-170,0.5,-84),T.Surface,"DDlg")
    dlg.ZIndex=81; mkCorner(dlg,12); mkStroke(dlg,T.Border,1); mkPad(dlg,14,14,14,14)
    mkLabel(dlg,"Set Emote Image",UDim2.new(1,0,0,22),UDim2.new(0,0,0,0),T.Accent,14).Font=Enum.Font.GothamBold
    mkLabel(dlg,"Paste a Decal asset ID to display as the emote thumbnail.",
        UDim2.new(1,0,0,26),UDim2.new(0,0,0,26),T.Sub,10).TextWrapped=true
    local db = mkBox(dlg,"Decal ID…",UDim2.new(1,0,0,30),UDim2.new(0,0,0,58))
    if em.decalId then db.Text = tostring(em.decalId) end; db.TextSize=12
    local function cl() pcall(function() ov:Destroy(); dlg:Destroy() end) end
    local sv = mkBtn(dlg,"Set Image",UDim2.new(0.48,-2,0,28),UDim2.new(0,0,1,-28),T.OK); sv.TextSize=11
    local ca = mkBtn(dlg,"Cancel",   UDim2.new(0.48,-2,0,28),UDim2.new(0.52,2,1,-28),T.Bad); ca.TextSize=11
    ca.MouseButton1Click:Connect(cl)
    sv.MouseButton1Click:Connect(function()
        local rawId = db.Text:match("^%s*(.-)%s*$")
        local parsed = extractId(rawId) or (rawId ~= "" and rawId or nil)
        em.decalId = parsed; cl()
        if onDone then onDone() end
        notify(parsed and ("Decal set: "..parsed) or "Image cleared","success",2)
    end)
end

-- ── Float a single emote ─────────────────────────────────────────────
local function floatEmote(em, idx)
    if em.floated and FloatingEmotes[idx] and FloatingEmotes[idx].Parent then return end
    em.floated = true

    -- Stagger position so multiple floats don't perfectly overlap
    local offset = 0
    for _ in pairs(FloatingEmotes) do offset += 1 end

    local fb = Instance.new("TextButton")
    fb.Size     = UDim2.new(0,72,0,80)
    fb.Position = UDim2.new(0, 10 + offset*12, 0.5, -40 + offset*18)
    fb.BackgroundColor3 = T.AccDark; fb.TextColor3 = T.Text
    fb.Text     = ""; fb.ZIndex = 90
    fb.BorderSizePixel = 0; fb.AutoButtonColor = false
    mkCorner(fb,14); mkStroke(fb,T.AccHov,2); fb.Parent = GUI

    -- Thumbnail image (if decalId set)
    local imgHolder = mkFrame(fb,UDim2.new(1,-4,0,48),UDim2.new(0,2,0,2),T.Surface); mkCorner(imgHolder,10)
    if em.decalId then
        local img = Instance.new("ImageLabel")
        img.Size = UDim2.new(1,0,1,0); img.BackgroundTransparency=1
        img.Image = "rbxassetid://"..tostring(em.decalId)
        img.ScaleType = Enum.ScaleType.Fit; img.Parent = imgHolder
    else
        local nl = mkLabel(imgHolder,"▶",UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),T.Accent,22)
        nl.TextXAlignment = Enum.TextXAlignment.Center
    end

    -- Name label
    local nl = mkLabel(fb,em.name:sub(1,9),UDim2.new(1,-4,0,14),UDim2.new(0,2,0,52),T.Text,9)
    nl.TextXAlignment = Enum.TextXAlignment.Center; nl.TextTruncate = Enum.TextTruncate.AtEnd

    -- Close (unfloat) button
    local xb = Instance.new("TextButton")
    xb.Size=UDim2.new(0,16,0,16); xb.Position=UDim2.new(1,-18,0,2)
    xb.BackgroundColor3=T.Bad; xb.TextColor3=Color3.new(1,1,1)
    xb.Text="✕"; xb.TextSize=9; xb.Font=Enum.Font.GothamBold
    xb.ZIndex=92; xb.BorderSizePixel=0; xb.AutoButtonColor=false
    mkCorner(xb,8); xb.Parent=fb

    FloatingEmotes[idx] = fb

    -- Drag — snaps to nearest edge on release
    makeDraggable(fb,fb,function()
        local v=vp(); local ap=fb.AbsolutePosition; local as=fb.AbsoluteSize
        fb.Position = UDim2.new(0,math.clamp(ap.X,0,v.X-as.X),0,math.clamp(ap.Y,0,v.Y-as.Y))
    end)

    -- Play on click (whole button, but not the X)
    fb.MouseButton1Click:Connect(function()
        local animator = AC.Animator
        if not animator then
            warn("[AnimStudio] Float play: no Animator available")
            return
        end
        local animId = NormalizeAnimationId(em.id)
        if not animId then
            warn("[AnimStudio] Float play: invalid animation id for '"..em.name.."'")
            return
        end
        local a2 = Instance.new("Animation"); a2.AnimationId = animId
        pcall(function() a2.Priority = Enum.AnimationPriority.Action4 end)
        local ok2, tr2 = pcall(function() return animator:LoadAnimation(a2) end)
        if ok2 and tr2 then
            tr2.Looped = false; tr2:Play()
        else
            warn("[AnimStudio] Float play: LoadAnimation failed for '"..em.name.."'")
        end
    end)
    ...
    xb.MouseButton1Click:Connect(function()
        em.floated = false
        FloatingEmotes[idx] = nil
        pcall(function() fb:Destroy() end)
        if showFloat then refreshEmotes() end
        notify("Unfloated: "..em.name,"info",2)
    end)
end

-- ── Main emote grid renderer ──────────────────────────────────────────
refreshEmotes = function()
    ...
    for i, em in ipairs(Emotes) do
        ...
        if showFloat and not em.floated   then continue end
        if f ~= "" and not em.name:lower():find(f,1,true) then continue end

        local card = mkFrame(EmScroll,UDim2.new(0,106,0,130),UDim2.new(0,0,0,0),T.Card)
        card.LayoutOrder = i; mkCorner(card,10)
        mkStroke(card, em.favorited and T.Warn or (em.floated and T.Info or T.Border), 1)

        -- ── Three-dots button (top right) ────────────────────────────
        local dotsBtn = Instance.new("TextButton")
        dotsBtn.Size   = UDim2.new(0,22,0,22); dotsBtn.Position = UDim2.new(1,-24,0,2)
        dotsBtn.BackgroundColor3 = T.Surface; dotsBtn.TextColor3 = T.Sub
        dotsBtn.Text = "⋮"; dotsBtn.TextSize = 16; dotsBtn.Font = Enum.Font.GothamBold
        dotsBtn.BorderSizePixel = 0; dotsBtn.ZIndex = 6; dotsBtn.AutoButtonColor = false
        mkCorner(dotsBtn,6); dotsBtn.Parent = card

        -- ── Favorite star (top left) ─────────────────────────────────
        if em.favorited then
            local fs = mkLabel(card,"★",UDim2.new(0,14,0,14),UDim2.new(0,4,0,4),T.Warn,12)
            fs.ZIndex = 6; fs.TextXAlignment = Enum.TextXAlignment.Center
        end

        -- ── Float badge ──────────────────────────────────────────────
        if em.floated then
            local fb2 = mkFrame(card,UDim2.new(0,32,0,13),UDim2.new(0,2,0,75),T.Info)
            mkCorner(fb2,4)
            local fl2 = mkLabel(fb2,"Float",UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),T.Bg,8)
            fl2.TextXAlignment = Enum.TextXAlignment.Center
        end

        -- ── Thumbnail area ───────────────────────────────────────────
        local th = mkFrame(card,UDim2.new(1,-4,0,70),UDim2.new(0,2,0,2),T.Surface); mkCorner(th,8)
        if em.decalId then
            local img = Instance.new("ImageLabel")
            img.Size = UDim2.new(1,0,1,0); img.BackgroundTransparency = 1
            img.Image = "rbxassetid://"..tostring(em.decalId)
            img.ScaleType = Enum.ScaleType.Fit; img.Parent = th
        else
            local ni = mkLabel(th,"No Preview",UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),T.Muted,9)
            ni.TextXAlignment = Enum.TextXAlignment.Center
        end

        -- ── Name label ───────────────────────────────────────────────
        local nl = mkLabel(card,em.name,UDim2.new(1,-4,0,14),UDim2.new(0,2,0,74),T.Text,9)
        nl.TextXAlignment = Enum.TextXAlignment.Center
        nl.TextTruncate   = Enum.TextTruncate.AtEnd

        -- ── Play button ──────────────────────────────────────────────
        local pb = mkBtn(card,"▶  Play",UDim2.new(1,-4,0,22),UDim2.new(0,2,1,-25),T.Accent)
        pb.TextSize = 10
        pb.MouseButton1Click:Connect(function()
            local animator = AC.Animator
            if not animator then
                warn("[AnimStudio] Emote play: no Animator — character may not be loaded")
                notify("Character not ready.","error"); return
            end
            local animId = NormalizeAnimationId(em.id)
            if not animId then
                warn("[AnimStudio] Emote play: invalid id for '"..em.name.."'")
                notify("Invalid animation ID.","error"); return
            end
            local a = Instance.new("Animation"); a.AnimationId = animId
            pcall(function() a.Priority = Enum.AnimationPriority.Action4 end)
            local ok, tr = pcall(function() return animator:LoadAnimation(a) end)
            if ok and tr then
                tr.Looped = false; tr:Play()
                notify("Playing: "..em.name,"info",2)
            else
                warn("[AnimStudio] Emote play: LoadAnimation failed for '"..em.name.."' ("..tostring(animId)..")")
                notify("Failed to play emote.","error")
            end
        end)
        ...
        dotsBtn.MouseButton1Click:Connect(function()
            closeContextMenu()
            -- Figure out where to anchor the menu
            local cardAbs = card.AbsolutePosition; local cardSz = card.AbsoluteSize
            local vpSz    = vp()
            local mx = cardAbs.X; local my = cardAbs.Y + cardSz.Y + 2
            if my + 175 > vpSz.Y then my = cardAbs.Y - 177 end
            if mx + 162 > vpSz.X then mx = vpSz.X - 165  end

            local menu = Instance.new("Frame")
            menu.Name  = "CtxMenu"; menu.Size = UDim2.new(0,160,0,0)
            menu.Position = UDim2.new(0,mx,0,my); menu.AutomaticSize = Enum.AutomaticSize.Y
            menu.BackgroundColor3 = T.Surface; menu.BorderSizePixel = 0; menu.ZIndex = 95
            mkCorner(menu,10); mkStroke(menu,T.Accent,1); mkPad(menu,4,4,4,4); mkList(menu,2)
            menu.Parent = GUI; activeContextMenu = menu

            local function menuItem(txt, col, action)
                local b = mkBtn(menu,txt,UDim2.new(1,0,0,28),UDim2.new(0,0,0,0),col or T.Card)
                b.TextSize=11; b.TextXAlignment=Enum.TextXAlignment.Left
                mkPad(b,0,6,0,10); b.ZIndex=96
                b.MouseButton1Click:Connect(function()
            ...
            -- Rename
            menuItem("✏  Rename", T.Card, function()
                showRenameDialog(em, function() refreshEmotes() end)
            end)

            -- Set / change decal image
            local decalLabel = em.decalId and "🖼  Change Image" or "🖼  Set Image (Decal ID)"
            menuItem(decalLabel, T.Card, function()
                showDecalDialog(em, function() refreshEmotes() end)
            end)

            -- Float / Unfloat
            if em.floated then
                menuItem("⊡  Unfloat", T.Card, function()
                    if FloatingEmotes[i] then
                        pcall(function() FloatingEmotes[i]:Destroy() end)
                        FloatingEmotes[i] = nil
                    end
                    em.floated = false; refreshEmotes()
                    notify("Unfloated: "..em.name,"info",2)
                end)
            else
                menuItem("⊞  Float on Screen", T.Card, function()
                    floatEmote(em, i); refreshEmotes()
            ...
            -- Favorite / Unfavorite
            if em.favorited then
                menuItem("✩  Remove Favorite", T.Card, function()
                    em.favorited = false; refreshEmotes()
                end)
            else
                menuItem("★  Add to Favorites", T.Warn, function()
                    em.favorited = true; refreshEmotes()
                    notify("Favorited: "..em.name,"success",2)
                end)
            end

            -- Delete
            menuItem("🗑  Delete", T.Bad, function()
                if FloatingEmotes[i] then
                    pcall(function() FloatingEmotes[i]:Destroy() end)
                    FloatingEmotes[i] = nil
                end
                local n = em.name; table.remove(Emotes,i); refreshEmotes()
                notify("Deleted: "..n,"warning",2)
            end)
        end)
    end
end

-- ── Add emote — uses full pipeline (same as Home tab) ─────────────────
AddEmBtn.MouseButton1Click:Connect(function()
    local ov  = mkFrame(GUI,UDim2.new(1,0,1,0),UDim2.new(0,0,0,0),Color3.new(0,0,0),"AOv")
    ov.BackgroundTransparency=0.45; ov.ZIndex=70
    local dlg = mkFrame(GUI,UDim2.new(0,360,0,180),UDim2.new(0.5,-180,0.5,-90),T.Surface,"AEDlg")
    dlg.ZIndex=71; mkCorner(dlg,12); mkStroke(dlg,T.Border,1); mkPad(dlg,14,14,14,14)
    mkLabel(dlg,"Add Emote",UDim2.new(1,0,0,22),UDim2.new(0,0,0,0),T.Accent,15).Font=Enum.Font.GothamBold
    mkLabel(dlg,"Name  (leave blank for auto):",UDim2.new(1,0,0,13),UDim2.new(0,0,0,26),T.Sub,10)
    local nb = mkBox(dlg,"Emote name…",UDim2.new(1,0,0,28),UDim2.new(0,0,0,42)); nb.TextSize=11
    mkLabel(dlg,"Animation ID or Roblox Link:",UDim2.new(1,0,0,13),UDim2.new(0,0,0,76),T.Sub,10)
    local ib = mkBox(dlg,"ID or link…",UDim2.new(1,0,0,28),UDim2.new(0,0,0,92)); ib.TextSize=11
    local function cl2() pcall(function() ov:Destroy(); dlg:Destroy() end) end
    local addB = mkBtn(dlg,"Import via Pipeline",UDim2.new(0.55,-2,0,28),UDim2.new(0,0,1,-28),T.OK); addB.TextSize=11
    local canB = mkBtn(dlg,"Cancel",UDim2.new(0.42,-2,0,28),UDim2.new(0.58,2,1,-28),T.Bad); canB.TextSize=11
    canB.MouseButton1Click:Connect(cl2)
    addB.MouseButton1Click:Connect(function()
        local rawInput = ib.Text:match("^%s*(.-)%s*$")
        if rawInput == "" then notify("Paste an ID or link first.","warning") return end
        local emoteName = nb.Text ~= "" and nb.Text or ("Emote "..(#Emotes+1))
        cl2()
        -- Run through the full 8-step pipeline just like the Home tab
        showPipelineDialog(rawInput, "Emote", function(ok, resolvedId, category)
            if ok then
                table.insert(Emotes,{id=resolvedId, name=emoteName, favorited=false, decalId=nil, floated=false})
                refreshEmotes()
                notify("Added emote: "..emoteName,"success",4)
            end
        end)
    end)
end)
...
PlayBtn2.MouseButton1Click:Connect(function()
    ...
    if EdClone then
        ...
        local an=h2:FindFirstChildOfClass("Animator"); if not an then return end
        local id=SavedAnims[SelectedSlot]; if not id or id=="" then return end
        local a=Instance.new("Animation"); a.AnimationId="rbxassetid://"..id
        pcall(function() a.Priority=SLOT_PRIORITY[SelectedSlot] or Enum.AnimationPriority.Action end)
        local ok,tr=pcall(function() return an:LoadAnimation(a) end)
        if ok and tr then tr:Play() else warn("[AnimStudio] Editor: LoadAnimation failed for "..id) end
    end
end)
...
for _, e in ipairs({
    ...
    table.insert(Emotes,{id=e[1], name=e[2], favorited=false, decalId=nil, floated=false})
end
refreshEmotes()
