-- ═══════════════════════════════════════════════════════
-- HUD SCRIPT — Auto Farm + ESP + Anti Death
-- ═══════════════════════════════════════════════════════
...
-- fill the lower half so bottom corners are flat
...
scrollFrame.Size   = UDim2.new(1, -24, 1, -182)  -- leaves room for title, tabs, anti-death
...
-- Creates a standard row: label on the left, toggle button on the right.
-- Returns: toggleButton, rowFrame
...
-- Small sub-label inside an existing row
...
-- Number input box inside an existing row
...
local afTeleportBtn, afTpRow   = makeToggleRow(afPanel, "Auto Teleport", 2, 32)
addSubLabel(afTpRow, "Set HP (TP below):", 41)
local afHPInput = addInputBox(afTpRow, 0, 39)
afHPInput.PlaceholderText = "HP (0 = off)"

local afAutoSellBtn, _ = makeToggleRow(afPanel, "Auto Sell (10 items)", 3)
-- Sell All button row
...
-- Auto teleport starts ON
applyToggleVisual(afTeleportBtn, true)
...
local autoTeleportOn = true
local autoSellOn     = false
local farmTask       = nil
local itemsCollected = 0
local afHPThreshold  = 0
...
-- Items to ignore by ancestor name
...
local function findFarmItems()
    ...
    -- Direct workspace children first (exact priority names)
...
local function sellAll()
    local remote = nil
    -- Search common locations for a sell remote
...
afHPInput.FocusLost:Connect(function()
    local v = tonumber(afHPInput.Text)
    if v and v >= 0 then afHPThreshold = v
    else afHPInput.Text = tostring(afHPThreshold) end
end)
...
local function startFarming()
    ...
    farmTask = task.spawn(function()
        while autoFarmOn do
            ...
            -- Set HP teleport (from afHPInput)
            if afHPThreshold > 0 then
                local hum2 = character:FindFirstChild("Humanoid")
                if hum2 and hum2.Health > 0 and hum2.Health <= afHPThreshold then
                    pcall(function() hrp.CFrame = CFrame.new(SAFE_POS) end)
                end
            end
            ...
            if item then
                ...
                if autoTeleportOn and dist > 5 then
                    tpToItem(item)
                    task.wait(0.05)
                end
...
afTeleportBtn.MouseButton1Click:Connect(function()
    autoTeleportOn = not autoTeleportOn
    applyToggleVisual(afTeleportBtn, autoTeleportOn)
end)
...
-- ── ESP Player (with capacity input) ─────────────────
...
-- Highlight storage: table of Highlight / BillboardGui instances keyed by category
...
-- Players: 10% opacity (0.90 trans) when very close → 20% opacity (0.80 trans) at mid → invisible at max
local CLOSE_STUD_THRESHOLD = 20
local function playerFillTrans(dist, maxD)
    if dist >= maxD then return 1 end
    if dist <= CLOSE_STUD_THRESHOLD then return 0.90 end  -- 10% opacity when close
    local t = (dist - CLOSE_STUD_THRESHOLD) / (maxD - CLOSE_STUD_THRESHOLD)
    return 0.80 + 0.20 * t  -- 20% opacity at mid, fades to invisible at maxD
end

-- Labels transparency tracks fill
local function playerTextTrans(dist, maxD)
    return math.clamp(playerFillTrans(dist, maxD), 0, 0.9)
end

-- NPCs: 0 (full) at ≤5 studs → 0.90 (10 % visible) at ≥70 studs
...
local function updatePlayerESP()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        local dist = (myRoot and root) and (myRoot.Position - root.Position).Magnitude or 999
        ...
        -- Item label sits on top (first half), username below (second half)
        if tool then
...
local function updateItemESP()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        ...
        if nameInListFuzzy(obj.Name, ITEM_NAMES) then
            ...
            elseif obj:IsA("Model") then adornee = obj
            end
...
-- ESP refresh loop — 1 second interval keeps NPCs updated after respawn
task.spawn(function()
    while true do
        task.wait(1)
        if espPlayerOn  then updatePlayerESP() end
        if espNPCOn     then updateNPCESP()    end
        if espItemsOn   then updateItemESP()   end
        if espExitOn    then updateExitESP()   end
    end
end)
    ...
    while true do
        ...
        if antiDeathOn then
            ...
            if char then
                ...
                local root = char:FindFirstChild("HumanoidRootPart")
                if hum and root and hum.Health > 0 and hum.Health <= tpThreshold then
                    pcall(function()
