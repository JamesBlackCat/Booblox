...
local function loadData()
    ...
    -- Fill missing fields with defaults (no longer forces ESP off —
    -- saved enabled state is restored so re-executing syncs back up)
...
-- Auto-restore: background scan for any game that had groups enabled at save time.
-- Runs 2 s after execute so the world is fully loaded before we walk it.
task.spawn(function()
    task.wait(2)
    rebuildPlayerChars()
    for gi, g in ipairs(NpcGames) do
        ...
        for _, grp in pairs(g.groups or {}) do
            if grp.espEnabled then hasEnabled = true ; break end
        end
        if hasEnabled then
            ...
            for gname, models in pairs(found) do
                ...
                for _, m in ipairs(models) do SpawnKnown[key][m] = true end
                local grp = g.groups and g.groups[gname]
                if grp and grp.espEnabled then
...
-- Shortens large HP numbers: 120000000 → "120M", 500000 → "500K", 99999 → "99999"
local function formatHP(n)
    n = math.floor(n or 0)
    if n >= 1000000000 then return math.floor(n / 1000000000) .. "B"
    elseif n >= 1000000 then return math.floor(n / 1000000) .. "M"
    elseif n >= 100000  then return math.floor(n / 1000) .. "K"
...
-- showHP is a new parameter appended after nameText.
-- Uses absolute pixel rows so every combination of enabled labels looks clean.
local function makeBillboard(root, showName, showDist, showHP, color, font, nameText)
    if not root then return nil end

    -- Measure total height needed
    local h = 4  -- base top-padding
    if showName then h = h + 22 end
    if showDist then h = h + 18 end
    if showHP   then h = h + 18 end
    h = math.max(h, 20)

    local bill = Instance.new("BillboardGui")
    bill.Size           = UDim2.new(0, 160, 0, h)
    bill.StudsOffset    = Vector3.new(0, 3.2, 0)
    bill.AlwaysOnTop    = true
    bill.LightInfluence = 0
    bill.Parent         = root

    local yOff = 2  -- start 2px from top

    if showName then
        local nl = Instance.new("TextLabel")
        nl.Name              = "NameLbl"
        nl.Size              = UDim2.new(1, 0, 0, 22)
        nl.Position          = UDim2.new(0, 0, 0, yOff)
    ...
    if showHP then
        ...
        hl.Name              = "HPLbl"
        hl.Size              = UDim2.new(1, 0, 0, 18)
        hl.Position          = UDim2.new(0, 0, 0, yOff)
        hl.BackgroundTransparency = 1
        hl.TextColor3        = Color3.fromRGB(120, 255, 120)  -- soft green for HP
        hl.Font              = font  or Enum.Font.GothamBold
        hl.TextSize          = 11
        hl.TextStrokeTransparency = 0.6
...
-- Creates actual Roblox instances for one already-registered model.
-- Called from the render loop once the model is within range.
local function createEspInstances(gi, gname, model, data)
    local g   = NpcGames[gi]
    ...
    if grp.espName or grp.espDistance then
        data.bill = makeBillboard(root, grp.espName, grp.espDistance, color, font, model.Name)
    end

    -- Outline = ring only (fill invisible)
    -- Body    = semi-transparent fill, no outline ring
    -- Both    = fill + ring
    if grp.espOutline or grp.espFullBody then
        local fillT    = grp.espFullBody and 0.5 or 1.0
        local outlineT = grp.espOutline  and 0.0 or 1.0
        data.hl = makeHighlight(model, color, color, fillT, outlineT)
    end

    data.pending = false
end

-- Registers models as pending (no instances yet). The render loop
-- creates instances once each model is within fadeStudDist range.
local function applyNpcGroupESP(gi, gname, models)
    ...
    for _, model in ipairs(models) do
        if not NpcESPActive[key][model] then
            NpcESPActive[key][model] = { pending=true, bill=nil, hl=nil, helpBill=nil, helpHl=nil }
...
-- Destroys all instances for a group and re-registers every known
-- model as pending. Call when ESP settings change for a group.
local function rebuildNpcGroupESP(gi, gname)
    local g   = NpcGames[gi]
    if not g then return end
    local grp = g.groups and g.groups[gname]
    ...
    if NpcESPActive[key] then
        for _, d in pairs(NpcESPActive[key]) do
            pcall(function()
                if d.bill     then d.bill:Destroy()     end
                if d.hl       then d.hl:Destroy()       end
                if d.helpBill then d.helpBill:Destroy() end
    ...
    -- If ESP is now disabled, stop here
    if not grp or not grp.espEnabled then return end

    -- Re-register all known models as pending (in-range ones
    -- will get instances on the next render frame)
    if SpawnKnown[key] then
        local models = {}
...
local function makeESPTypePills(grp, parent, onChanged)
    ...
    for i, t in ipairs(types) do
        ...
        pill.Position         = UDim2.new(0, 52 + (i-1)*46, 0.5, -11)
...
ScanBtn.MouseButton1Click:Connect(function()
    ...
    for gname, models in pairs(found) do
        ...
        -- Apply ESP immediately for groups that already have it enabled
        local grp = g.groups[gname]
        if grp and grp.espEnabled then
            applyNpcGroupESP(CurrentGameIdx, gname, models)
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    -- NPC ESP — create pending instances when in range (max 3/frame)
    local pendingBudget = 3
    for gi, g in ipairs(NpcGames) do
        for gname, grp in pairs(g.groups or {}) do
                    ...
                    for model, data in pairs(active) do
                        if model and model.Parent then
                            ...
                            if root then
                                ...
                                local fadeDist  = grp.fadeStudDist or 200
                                -- 30 % hysteresis buffer stops boundary thrashing:
                                -- create at fadeStudDist, destroy at fadeStudDist * 1.3
                                local destroyAt = fadeDist * 1.3

                                if data.pending then
                                    -- Materialise when the model is in range and budget allows
                                    if pendingBudget > 0 and dist <= fadeDist then
                                        createEspInstances(gi, gname, model, data)
                                        pendingBudget = pendingBudget - 1
                                    end
                                else
                                    -- Out-of-range: destroy instances → reset to pending.
                                    -- Only live instances for nearby NPCs = huge GPU/CPU saving.
                                    if dist > destroyAt then
                                        if data.bill     then data.bill:Destroy()     ; data.bill     = nil end
                                        if data.hl       then data.hl:Destroy()       ; data.hl       = nil end
                                        ...
                                        -- Normal per-frame update for in-range models
                                        if data.bill then
                                        ...
                                        if grp.helpEnabled then
                                            ...
                                            if hp < grp.helpHP then
                                                ...
                                                local hs = {
                                                    ...
                                                    fadeStudDist    = grp.helpFadeDist,
                                                    fadeNameDist    = grp.helpFadeDist,
                                                    fadeCloseDist   = grp.fadeCloseDist,
                                                    fadeCloseNameDist = grp.fadeCloseNameDist,
                                                }
                ...
                if pchar and phrp then
                    ...
                    if not PlayerBills[uid] then
                        ...
                        if pd.espOutline or pd.espFullBody then
                            local fillT    = pd.espFullBody and 0.5 or 1.0
                            local outlineT = pd.espOutline  and 0.0 or 1.0
                            hl = makeHighlight(pchar, color, color, fillT, outlineT)
                        end
...
--  Replaces polling loops — zero recurring scans.
--  Fires instantly when a Humanoid enters workspace.
-- ══════════════════════════════════════════
local function onNewHumanoid(hum)
    local model = hum.Parent
    if not model or not model:IsA("Model") then return end
    if playerChars[model] then return end

    local gname = model.Name
    ...
    for gi, g in ipairs(NpcGames) do
        ...
        if grp then
            ...
            local isNew = not SpawnKnown[key][model]
            SpawnKnown[key][model] = true

            -- Spawn notify
            if isNew and grp.spawnEnabled then
                -- Wait a frame so the model is fully loaded before
                -- we try to find its HumanoidRootPart
                task.defer(function()
            ...
            -- Add to active ESP if enabled
            if grp.espEnabled then
                task.defer(function()
                    if model.Parent then
                        applyNpcGroupESP(gi, gname, {model})
                    end
                end)
            end
        end
...
-- Listen for Humanoids added anywhere under workspace
