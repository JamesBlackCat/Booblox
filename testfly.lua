...
-- Predicted aim point for `plr` — Smart-AI lead based on the assembly's
-- linear velocity and the time it would take a "shot" (or eyes) to traverse
-- the world distance from the camera. Falls back to raw position when AI off.
local function predictedAimPoint(plr, hrp)
    ...
    if v.Magnitude < 0.5 then return part.Position, part end
    -- Use time-to-target based on distance / nominal projectile speed (1000 st/s).
    -- This works as a tracking lead even when not actually firing a bullet.
    ...
    return part.Position + v * leadT, part
end
...
local function makeCrosshairGui()
    ...
    img.Image              = "rbxassetid://107058246184363"  -- HARDCODED crosshair icon
...
RunService:BindToRenderStep("FlyAimlock", Enum.RenderPriority.Camera.Value + 2, function()
    ...
    -- Build target CFrame and apply smoothing / per-frame angular cap.
...
local function buildUI()
    ...
    AT:CreateInput({ Name = "Target Body Part", PlaceholderText = "Head / HumanoidRootPart / Torso",
    ...
    AT:CreateSection("Crosshair Position  (icon is built-in and cannot be changed)")
