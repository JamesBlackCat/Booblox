...
local function getKillParts()
    local tower = getTower()
    if not tower then return {} end
    -- Primary: workspace.tower.sections.killPart
    local sections = tower:FindFirstChild("sections")
    if sections then
        local kp = sections:FindFirstChild("killPart")
        if kp then
            local parts = {}
            for _, d in ipairs(kp:GetDescendants()) do
                if d:IsA("BasePart") then table.insert(parts, d) end
            end
            if #parts > 0 then return parts end
        end
    end
    -- Fallback: search all tower descendants for touch parts
    local parts = {}
...
mrEvilBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(10, function()
        ...
        particles.Texture       = "rbxassetid://72606765272621"
        ...
        particles.RotSpeed      = NumberRange.new(-120, 120)
