...
local D = {
    ...
    craterPartMinSize         = 8,     -- studs — only parts whose LARGEST axis is ≥ this get craterized
                                        -- (use largest axis so thin huge baseplates still qualify)
    craterPartMaxSize         = 2000,  -- studs — don't try to craterize ridiculously big parts
...
-- ── Debris registry + global cap (anti-lag) ────────────────
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- Phase-through hitbox (now SPHERICAL — radius = phaseHitboxSize / 2)
    ...
    -- skip Water cells (per user request — no draining oceans).
