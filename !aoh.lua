...
makeToggle("Camera Phase-Through", 4, function(on)
    ...
    if not on then
        ...
        -- Create 4-stud invisible hitbox that follows the camera
        camHitbox = Instance.new("Part")
        camHitbox.Size        = Vector3.new(4, 4, 4)
...
RunService.RenderStepped:Connect(function()
    ...
    -- Cast from camera toward HRP, fade any part within 4 studs of camera
    ...
    if ray and ray.Instance and ray.Instance:IsA("BasePart") then
        ...
        -- Also fade parts within 4 studs of camera itself
        local camDist = (origin - ray.Position).Magnitude
        local alpha = camDist < 4 and 1 or 0.88
        phasedParts[part] = 0
...
-- 5. Invisible Players
local invisOriginalTrans = {}  -- [BasePart/Decal] = originalTransparency
...
local function applyInvis(plr, on)
    ...
    if isWhitelisted and on then return end
    local c = plr.Character
    if not c then return end
    for _, desc in ipairs(workspace:GetDescendants()) do
        local owner = Players:GetPlayerFromCharacter(desc.Parent)
        if owner == plr then
            pcall(function()
                if desc:IsA("BasePart") or desc:IsA("Decal") then
                    if on then
                        ...
                        if invisOriginalTrans[desc] then
                            desc.Transparency = invisOriginalTrans[desc]
                            invisOriginalTrans[desc] = nil
                        end
                    end
                end
                if desc:IsA("BillboardGui") then desc.Enabled = not on end
            end)
        end
    end
end
...
-- 6. Render Distance  (simulated via fog: 1% = 30 studs, 100% = no fog limit)
makeSlider("Render Distance %", 11, 1, 100, 100, function(val)
    State.renderDist = val
    if val >= 100 then
        -- Full distance — restore original fog so nothing is clipped
        Lighting.FogColor = originalFog.FogColor
        Lighting.FogStart = originalFog.FogStart
        Lighting.FogEnd   = originalFog.FogEnd
    else
        local studs = val * 30  -- 1% = 30 studs, 100% = 3000 studs
        Lighting.FogColor = Color3.fromRGB(20, 20, 25)
        Lighting.FogStart = math.max(0, studs - 20)
...
floodBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(10, function()
        ...
        -- Spawn flood at lobby.Chill if it exists, else below player
        ...
        pcall(function()
            ...
            if tower then
                local sections = tower:FindFirstChild("sections")
                if sections then
                    local lobby = sections:FindFirstChild("lobby")
                    if lobby then
...
mrEvilBtn.MouseButton1Click:Connect(function()
    ...
    task.delay(10, function()
        ...
        -- 1.5× player scale: Roblox player ≈ 2×5×1 → Mr Evil ≈ 3×7.5×1.5
        local body = Instance.new("Part")
        body.Size        = Vector3.new(3, 7.5, 1.5)
        ...
        body.Position    = h.Position + Vector3.new(0, 0, 30)  -- spawn behind player (Z axis)
        ...
        pcall(function() particles.Texture = "rbxassetid://72606765272621" end)
        pcall(function()
...
rlglBtn.MouseButton1Click:Connect(function()
    ...
    redLightConn = RunService.Heartbeat:Connect(function()
        ...
        -- 1.5s grace period after red starts before movement is punished
        if isRed and lastPos and (now - redStartTime) > 1.5 then
