...
local D = {
    ...
    -- ── FE Impacts ──────────────────────────────────────────
    impactsEnabled            = true,
    impactMinSpeed            = 40,    -- minimum landing speed to trigger any impact
    impactSlideEnabled        = true,  -- violent ongoing impact while sliding along ground
    impactSlideRate           = 0.05,  -- how often (sec) slide impact emits
    impactSpeedScaleStart     = 200,   -- speed at which sizes start scaling up
    impactSpeedScaleMaxSpeed  = 10000, -- speed that maps to maxRadius
    impactMaxRadius           = 2000,  -- max impact radius in studs at top speed
    impactBaseRadius          = 6,     -- base radius at min speed
    -- toggles per effect
    fxShockwaveEnabled        = true,
    fxDebrisEnabled           = true,
    fxCracksEnabled           = true,
    fxDustEnabled             = true,
    fxEnergyPulseEnabled      = true,
    fxSpeedTrailEnabled       = true,  -- speed-based effect (continuous trail)
    ...
    fxDustSize                = 1,
    fxEnergyPulseSize         = 1,
    -- speed thresholds (each effect only triggers at/above this speed)
    ...
    -- debris settings
    debrisChunksMax           = 20,    -- chunks per impact at top size
    debrisSmallCount          = 12,
    debrisMediumCount         = 6,
    debrisLargeCount          = 3,
    debrisDespawnSec          = 8,     -- 1..600 seconds (10 min)
    debrisCanCollide          = true,
    -- crater / digging
    craterEnabled             = true,
    craterDepthMul            = 1,     -- depth scales with impact radius
    digTerrainWhileFlying     = true,  -- continuously dig terrain while flying through it
    digMinSpeed               = 80,    -- minimum speed to dig terrain
    impactAffectParts         = true,  -- create cracks/decals on bricks too
    -- phase-through small parts
    phaseEnabled              = true,
    phaseMaxPartSize          = 12,    -- studs (longest axis) — parts at/under this get phased
    phaseLeaveHole            = true,  -- destroy/break the part on entry
    phaseMinSpeed             = 60,    -- minimum speed to phase through
    -- Keybinds
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- Higher target speeds (>100) progressively slow the ramp so reaching
    -- top speed is a gradual climb instead of a snap.
    local accelMul = rawAccel / 50
    local speedSlowdown = 1
    if targetSpeed > 100 then
        speedSlowdown = math.clamp(100 / targetSpeed, 0.05, 1)
    end
    -- Final per-second lerp rate (baseline 8 ≈ original "fast" feel)
    ...
    local effectiveTarget = targetSpeed
...
local function fxDebris(pos, normal, radius, surfaceColor, surfaceMaterial)
    ...
    for _, cat in ipairs(categories) do
        for _ = 1, cat.count do
            ...
            p.CanQuery   = false
            p.CFrame     = CFrame.new(pos + normal * 0.5)
...
local function tryPhasePart(part, hrp, speed)
    ...
    if part.Anchored == false and part.AssemblyMass <= 0 then return end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- Forward raycast for phase-through (only while flying)
    if State.flying and S.phaseEnabled and speed >= S.phaseMinSpeed then
        ...
        if fwd.Magnitude > 0.01 then
            local dir = fwd.Unit * math.clamp(speed * dt * 1.5 + 4, 4, 60)
            local hit = workspace:Raycast(hrp.Position, dir, rayParams)
            if hit and hit.Instance and hit.Instance:IsA("BasePart") then
                tryPhasePart(hit.Instance, hrp, speed)
            end
        end
    end
    ...
    if grounded and S.impactSlideEnabled and State.flying and speed >= S.impactMinSpeed then
        if tick() - _lastSlideTime >= S.impactSlideRate then
            _lastSlideTime = tick()
            -- Slide impacts use a fraction of the regular debris/dust counts
            local pos    = downHit.Position
            local normal = downHit.Normal
...
local function buildUI()
    ...
    --  FE IMPACTS TAB
    -- ────────────────────────────────
    local IT = Window:CreateTab("FE Impacts", 4483362458)

    IT:CreateSection("Master")
    IT:CreateToggle({ Name = "Enable Impact System", CurrentValue = S.impactsEnabled, Flag = "ImpactsOn",
        Callback = function(v) S.impactsEnabled = v end })
    ...
    IT:CreateToggle({ Name = "Enable Slide Impact", CurrentValue = S.impactSlideEnabled, Flag = "SlideOn",
        Callback = function(v) S.impactSlideEnabled = v end })
    ...
    IT:CreateToggle({ Name = "Enable Shockwave", CurrentValue = S.fxShockwaveEnabled, Flag = "FXShockOn",
        Callback = function(v) S.fxShockwaveEnabled = v end })
    IT:CreateSlider({ Name = "Trigger Speed", Range = {0,2000}, Increment = 5, Suffix = "/s", CurrentValue = S.fxShockwaveTrigger, Flag = "FXShockTrig",
        Callback = function(v) S.fxShockwaveTrigger = v end })
    IT:CreateSlider({ Name = "Size Multiplier", Range = {1,10}, Increment = 1, CurrentValue = S.fxShockwaveSize, Flag = "FXShockSize",
        Callback = function(v) S.fxShockwaveSize = v end })

    IT:CreateSection("Debris Burst")
    IT:CreateToggle({ Name = "Enable Debris", CurrentValue = S.fxDebrisEnabled, Flag = "FXDebrisOn",
        Callback = function(v) S.fxDebrisEnabled = v end })
    IT:CreateSlider({ Name = "Trigger Speed", Range = {0,2000}, Increment = 5, Suffix = "/s", CurrentValue = S.fxDebrisTrigger, Flag = "FXDebrisTrig",
        Callback = function(v) S.fxDebrisTrigger = v end })
    IT:CreateSlider({ Name = "Size Multiplier", Range = {1,10}, Increment = 1, CurrentValue = S.fxDebrisSize, Flag = "FXDebrisSize",
    ...
    IT:CreateToggle({ Name = "Debris Can Collide", CurrentValue = S.debrisCanCollide, Flag = "DebrisColl",
    ...
    IT:CreateToggle({ Name = "Enable Cracks", CurrentValue = S.fxCracksEnabled, Flag = "FXCracksOn",
        Callback = function(v) S.fxCracksEnabled = v end })
    IT:CreateSlider({ Name = "Trigger Speed", Range = {0,2000}, Increment = 5, Suffix = "/s", CurrentValue = S.fxCracksTrigger, Flag = "FXCracksTrig",
        Callback = function(v) S.fxCracksTrigger = v end })
    IT:CreateSlider({ Name = "Size Multiplier", Range = {1,10}, Increment = 1, CurrentValue = S.fxCracksSize, Flag = "FXCracksSize",
        Callback = function(v) S.fxCracksSize = v end })

    IT:CreateSection("Dust Explosion")
    IT:CreateToggle({ Name = "Enable Dust", CurrentValue = S.fxDustEnabled, Flag = "FXDustOn",
        Callback = function(v) S.fxDustEnabled = v end })
    IT:CreateSlider({ Name = "Trigger Speed", Range = {0,2000}, Increment = 5, Suffix = "/s", CurrentValue = S.fxDustTrigger, Flag = "FXDustTrig",
        Callback = function(v) S.fxDustTrigger = v end })
    IT:CreateSlider({ Name = "Size Multiplier", Range = {1,10}, Increment = 1, CurrentValue = S.fxDustSize, Flag = "FXDustSize",
        Callback = function(v) S.fxDustSize = v end })

    IT:CreateSection("Energy Pulse")
    IT:CreateToggle({ Name = "Enable Energy Pulse", CurrentValue = S.fxEnergyPulseEnabled, Flag = "FXPulseOn",
        Callback = function(v) S.fxEnergyPulseEnabled = v end })
    IT:CreateSlider({ Name = "Trigger Speed", Range = {0,2000}, Increment = 5, Suffix = "/s", CurrentValue = S.fxEnergyPulseTrigger, Flag = "FXPulseTrig",
        Callback = function(v) S.fxEnergyPulseTrigger = v end })
    IT:CreateSlider({ Name = "Size Multiplier", Range = {1,10}, Increment = 1, CurrentValue = S.fxEnergyPulseSize, Flag = "FXPulseSize",
        Callback = function(v) S.fxEnergyPulseSize = v end })

    IT:CreateSection("Speed Trail  (continuous high-speed effect)")
    IT:CreateToggle({ Name = "Enable Speed Trail", CurrentValue = S.fxSpeedTrailEnabled, Flag = "FXTrailOn",
        Callback = function(v) S.fxSpeedTrailEnabled = v end })
    IT:CreateSlider({ Name = "Trigger Speed", Range = {0,2000}, Increment = 5, Suffix = "/s", CurrentValue = S.fxSpeedTrailTrigger, Flag = "FXTrailTrig",
        Callback = function(v) S.fxSpeedTrailTrigger = v end })

    IT:CreateSection("Crater  (fills terrain with Air at the impact site)")
    IT:CreateToggle({ Name = "Enable Crater", CurrentValue = S.craterEnabled, Flag = "CraterOn",
        Callback = function(v) S.craterEnabled = v end })
    ...
    IT:CreateToggle({ Name = "Enable Terrain Dig", CurrentValue = S.digTerrainWhileFlying, Flag = "DigOn",
        Callback = function(v) S.digTerrainWhileFlying = v end })
    ...
    IT:CreateSection("Phase-Through Small Parts  (fly through bricks at/under size)")
    IT:CreateToggle({ Name = "Enable Phase-Through", CurrentValue = S.phaseEnabled, Flag = "PhaseOn",
        Callback = function(v) S.phaseEnabled = v end })
    IT:CreateSlider({ Name = "Max Part Size to Phase (longest axis, studs)", Range = {1,200}, Increment = 1, Suffix = " st", CurrentValue = S.phaseMaxPartSize, Flag = "PhaseMaxSz",
    ...
    IT:CreateToggle({ Name = "Leave Hole (destroy/break the part on entry)", CurrentValue = S.phaseLeaveHole, Flag = "PhaseHole",
