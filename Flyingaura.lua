...
local D = {
    ...
    -- ≥ craterPartMinSize, the part is split into many smaller pieces forming
    -- a crater pattern instead of just leaving a crack decal.
    craterPartEnabled         = false,
    craterPartMinSize         = 12,    -- studs — only parts with min axis ≥ this get craterized
    craterPartMaxSize         = 1000,  -- studs — don't try to craterize ridiculously big parts
    craterPartChunkSize       = 4,     -- studs — approx voxel chunk size when splitting
    craterPartMinSpeed        = 80,    -- only at hard impact speeds
    ...
    aimlockFriendCheck        = false, -- skip Roblox friends
    aimlockWallCheck          = true,
    aimlockShowFOV            = false, -- draw the FOV circle on screen when aimlock is on
...
local function fxCraterOnPart(part, pos, normal, radius, speed)
    ...
    if minAxis < (S.craterPartMinSize or 12) then return end
    if maxAxis > (S.craterPartMaxSize or 1000) then return end
    ...
    --  by walking the parent chain — fall back gracefully if unavailable.)
    ...
    for ix = 0, nx - 1 do
        for iy = 0, ny - 1 do
            for iz = 0, nz - 1 do
                ...
                if d > craterR then
                    -- Outside crater radius — keep this chunk as static debris
                    local p = Instance.new("Part")
                    p.Anchored   = true
                    p.CanCollide = true
                    p.CanQuery   = false
                    ...
                    -- Inside the crater — fling a few chunks for impact spectacle,
                    ...
                    if math.random() < 0.18 then
                        ...
                        safeDestroy(p, S.debrisDespawnSec or 8)
                    end
                end
            end
        end
...
-- map geometry — never on rigs, zombies, or other characters.
...
local function emitImpact(pos, normal, speed, surfacePart)
    ...
    -- Ignore hits on players / rigs / NPCs — impacts only on bricks/parts.
...
local function setGreenLighting(on)
    if on and not Stages.greenLightingActive then
        ...
        -- HARD GREEN — fully saturated alien atmosphere
...
local Aimlock = {
    ...
    -- Friend cache: [userId] = bool. Refreshed when players join/leave.
    friendCache      = {},
}
...
local function isTargetLocked(plr)
    if not S.targetLockEnabled then return false end
...
-- Picks a target according to:
--   1) any player in the FOV who is on the Target Lock list (priority — bypasses
--      team check AND friend check)
--   2) otherwise, the closest player to the crosshair, respecting team / friend / wall checks.
local function pickAimTarget()
    local _, hrp = getCharParts()
    if not hrp then return nil end
    local cross = crosshairScreenPos()

    local bestPriority, bestPriorityDist = nil, math.huge
            ...
            if part then
                ...
                if worldD <= S.aimlockRange then
                    ...
                    if on and sp.Z > 0 then
                        ...
                        if pixD <= S.aimlockFOV and not wallBlocked(hrp.Position, part) then
                            local locked = isTargetLocked(p)
                            if locked then
                                -- Priority list — beats friend/team checks.
                                if pixD < bestPriorityDist then
                                    bestPriorityDist, bestPriority = pixD, p
                                end
                            else
                                local skip = false
                                if S.aimlockTeamCheck and p.Team and LocalPlayer.Team
                                and p.Team == LocalPlayer.Team then
                                    skip = true
                                end
                                if not skip and S.aimlockFriendCheck and isFriend(p) then
                                    skip = true
                                end
                                if not skip and pixD < bestNormalDist then
                                    bestNormalDist, bestNormal = pixD, p
                                end
                            end
                        end
                    end
...
-- Also creates the FOV ring (only displayed when S.aimlockShowFOV is true).
local function makeCrosshairGui()
    ...
    -- FOV ring (transparent circle scaled to S.aimlockFOV pixel radius)
    ...
    ring.Image             = "rbxassetid://3570695787"  -- transparent ring sprite
    ring.ImageColor3       = Color3.fromRGB(255, 60, 60)
    ring.ImageTransparency = 0.35
    ring.ScaleType         = Enum.ScaleType.Slice
    ring.SliceCenter       = Rect.new(100, 100, 100, 100)
...
RunService:BindToRenderStep("FlyAimlock", Enum.RenderPriority.Camera.Value + 2, function()
    ...
    -- Update FOV ring (visibility, position, size — all configurable live)
...
local function buildUI()
    ...
    AT:CreateToggle({ Name = "Show FOV (draw the FOV ring)", CurrentValue = S.aimlockShowFOV, Flag = "AimShowFOV",
    ...
    AT:CreateSection("Target Lock  (priority list — beats friend/team check)")
    AT:CreateToggle({ Name = "Enable Target Lock List", CurrentValue = S.targetLockEnabled, Flag = "TLOn",
        Callback = function(v) S.targetLockEnabled = v end })
    -- Player picker (refreshed every time the dropdown is opened-ish via SetOptions)
    local function _playerOptions()
        local out = {}
        for _, p in ipairs(Players:GetPlayers()) do
    ...
    local tlAddDD
    tlAddDD = AT:CreateDropdown({
        Name = "Add Player to Target Lock List",
        ...
        Callback = function(opt)
            ...
            if pick and pick ~= "" then
                S.targetLockList[string.lower(pick)] = true
                Rayfield:Notify({ Title = "Target Lock", Content = pick .. " added.", Duration = 3 })
            end
        end,
    })
    AT:CreateButton({ Name = "Refresh Player List", Callback = function()
        if tlAddDD and tlAddDD.Refresh then pcall(function() tlAddDD:Refresh(_playerOptions(), false) end) end
    end })
    local tlRemDD
    local function _listedOptions()
        local out = {}
        for name, _ in pairs(S.targetLockList) do table.insert(out, name) end
        table.sort(out)
        return out
    end
    tlRemDD = AT:CreateDropdown({
        Name = "Remove Player from List",
        Options = _listedOptions(),
        CurrentOption = "",
        Flag = "TLRem",
        Callback = function(opt)
            local pick = type(opt) == "table" and opt[1] or opt
            if pick and pick ~= "" then
                S.targetLockList[string.lower(pick)] = nil
                Rayfield:Notify({ Title = "Target Lock", Content = pick .. " removed.", Duration = 3 })
                if tlRemDD and tlRemDD.Refresh then pcall(function() tlRemDD:Refresh(_listedOptions(), false) end) end
            end
        end,
    })
    AT:CreateButton({ Name = "Clear Target Lock List", Callback = function()
        S.targetLockList = {}
        if tlRemDD and tlRemDD.Refresh then pcall(function() tlRemDD:Refresh(_listedOptions(), false) end) end
        Rayfield:Notify({ Title = "Target Lock", Content = "List cleared.", Duration = 3 })
    end })
