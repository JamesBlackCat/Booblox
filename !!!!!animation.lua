...
-- ============================================================
-- DRAGGABLE
-- Only snaps / calls onReleased if the user actually dragged
-- (moved more than DRAG_THRESHOLD pixels). Plain taps/clicks
-- are left alone so they don't interfere with button events.
-- ============================================================
...
local function makeDraggable(handle, target, onReleased)
    ...
    UserInputService.InputChanged:Connect(function(inp)
        ...
        if inp.UserInputType == Enum.UserInputType.MouseMovement or
           inp.UserInputType == Enum.UserInputType.Touch then
            local delta = getPos(inp) - startMouse
            -- Only start actually moving after threshold to avoid accidental drift
            if not hasMoved and delta.Magnitude < DRAG_THRESHOLD then return end
            hasMoved = true
    ...
    UserInputService.InputEnded:Connect(function(inp)
        ...
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or
           inp.UserInputType == Enum.UserInputType.Touch then
            local wasDrag = hasMoved
            dragging  = false
            hasMoved  = false
            -- Only snap/callback if the user actually dragged the element
            if wasDrag and onReleased then
                onReleased()
            end
        end
    end)
...
-- Hover: color only — no size change (size change shifts position without AnchorPoint 0.5)
ToggleBtn.MouseEnter:Connect(function()
    tween(ToggleBtn, {BackgroundColor3 = THEME.AccentHov}, 0.12)
end)
ToggleBtn.MouseLeave:Connect(function()
    tween(ToggleBtn, {BackgroundColor3 = THEME.Accent}, 0.12)
end)

-- Snap to nearest vertical edge after a real drag (not a tap)
makeDraggable(ToggleBtn, ToggleBtn, function()
    ...
    -- Keep Y as a pure pixel offset from top so it doesn't jump
    local newY  = math.clamp(ap.Y, 10, vSize.Y - as.Y - 10)
    if cx < vSize.X / 2 then
        tween(ToggleBtn, {Position = UDim2.new(0, 10, 0, newY)}, 0.2)
    else
        tween(ToggleBtn, {Position = UDim2.new(0, vSize.X - as.X - 10, 0, newY)}, 0.2)
    end
end)
...
-- Use Activated — works reliably on both PC (mouse) and mobile (touch)
ToggleBtn.Activated:Connect(function()
    if State.UIOpen then closeUI() else openUI() end
end)
CloseWin.Activated:Connect(closeUI)
