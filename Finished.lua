...
local function enterStage(n)
    ...
    if hrp then
        ...
        local body = spawnBody(data.body, bodyCF, data.bodySize)
    ...
    if n == 4 then
        ...
        makeArrowGui(function()
            if Stages.planetModel and Stages.planetModel.PrimaryPart then
                return Stages.planetModel.PrimaryPart.Position
            end
        end)
    end
...
-- Stages heartbeat: distance tracking + transitions
RunService.Heartbeat:Connect(function()
    ...
    if Stages.lastPos then
        local d = (pos - Stages.lastPos).Magnitude
        if State.flying and d < 500 then  -- ignore teleport jumps
            if Stages.current == 1 then
                if farFromMap(pos) then
                    Stages.distInStage = Stages.distInStage + d
                end
            else
                Stages.distInStage = Stages.distInStage + d
            end
    ...
    if Stages.studsLabel then
        Stages.studsLabel.Text = math.floor(Stages.distInStage) .. " studs"
    end

    -- Stage transitions
    if Stages.current == 1 and Stages.distInStage >= S.stage1ToStage2Studs then
        enterStage(2)
    elseif Stages.current == 2 then
        -- back to stage 1 if reached Earth
        if Stages.earthModel and Stages.earthModel.PrimaryPart then
            local dE = (pos - Stages.earthModel.PrimaryPart.Position).Magnitude
            if dE < (S.earthReturnDist + (Stages.earthModel:GetExtentsSize().Magnitude * 0.5)) then
                enterStage(1)
                return
            end
        end
        if Stages.distInStage >= S.stage2ToStage3Studs then enterStage(3) end
    elseif Stages.current == 3 then
        if Stages.solarModel and Stages.solarModel.PrimaryPart then
            local dS = (pos - Stages.solarModel.PrimaryPart.Position).Magnitude
            if dS < (S.solarReturnDist + (Stages.solarModel:GetExtentsSize().Magnitude * 0.5)) then
                enterStage(2)
                return
            end
        end
        if Stages.distInStage >= S.stage3ToStage4Studs then enterStage(4) end
    elseif Stages.current == 4 then
        if Stages.planetModel and Stages.planetModel.PrimaryPart then
            local dP = (pos - Stages.planetModel.PrimaryPart.Position).Magnitude
            if dP < (S.planetExitDist + (Stages.planetModel:GetExtentsSize().Magnitude * 0.5)) then
                -- Teleport to spawn
...
local function attachAlienHat(plr)
    ...
    local function onChar(char)
        ...
        if not objs then return end
        for _, obj in ipairs(objs) do
            local acc = obj:IsA("Accessory") and obj or obj:FindFirstChildWhichIsA("Accessory")
            if acc then
