...
local function loadAnim(rawId, animr)
    ...
    -- Try to load asset directly — could be Animation, KeyframeSequence,
    -- a Model containing Animations (full Roblox animation packages),
    -- or a catalog emote bundle (Folder of Animations).
    local foundAnim
    local ownsAnim  = false  -- true if we created the Animation ourselves
    ...
    if ok and objects then
        for _, obj in ipairs(objects) do
            ...
            -- Recursively search inside Models / Folders / Animation Packages
            local nested = obj:FindFirstChildWhichIsA("Animation", true)
            if nested then
                foundAnim = nested
                break
            end
    ...
    -- Fallback: treat the numeric ID as a direct animation asset
    if not foundAnim then
        foundAnim = Instance.new("Animation")
        foundAnim.AnimationId = "rbxassetid://" .. numId
        ownsAnim = true
    end

    local ok2, track = pcall(function()
        local t = animr:LoadAnimation(foundAnim)
        t.Priority = Enum.AnimationPriority.Action4
        return t
    end)
    if ownsAnim then pcall(function() foundAnim:Destroy() end) end
    ...
    if S.flyAnimEnabled then
        ...
        if State.flying then
            if isMoving then
                ...
                if S.directionalAnimsEnabled then
                    local camCF   = Camera.CFrame
