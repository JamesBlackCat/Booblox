...
local afTeleportBtn, _ = makeToggleRow(afPanel, "Auto Teleport",  2)
...
local function startFarming()
    ...
    farmTask = task.spawn(function()
        while autoFarmOn do
            ...
            if item then
                afStatusLbl.Text = "Status: Farming " .. item.Name
                if autoTeleportOn and dist > 5 then
                    tpToItem(item)
                    task.wait(0.05)
                end
                local ok = collectItem(item)
                if not ok and dist < 3 then
                    stuckItems[item] = true
                end
            else
                afStatusLbl.Text = "Status: No items found"
                task.wait(1)
            end
            task.wait(0.05)
        end
...
-- Players: base 0.60 at close range → 1 (invisible) at max studs
...
local function updatePlayerESP()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if tool then
            ...
            itemLbl.Position           = UDim2.new(0, 0, 0.5, 0)
