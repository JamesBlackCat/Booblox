...
local function makeCrosshairGui()
    ...
    ring.ImageColor3       = Color3.fromRGB(S.aimlockFOVColorR or 255, S.aimlockFOVColorG or 60, S.aimlockFOVColorB or 60)
    ...
    img.Image              = S.aimlockCrosshairImg or "rbxassetid://107058246184363"
...
RunService:BindToRenderStep("FlyAimlock", Enum.RenderPriority.Camera.Value + 2, function()
    ...
    if Aimlock.fovCircle then
        ...
        Aimlock.fovCircle.ImageColor3 = Color3.fromRGB(
    ...
    -- ── CURSOR-AWARE AIMING ──
    -- When Cursor Mode is ON: build a camera CFrame such that `aimPos`
    -- projects to the CROSSHAIR pixel (not screen center). Compute the
    -- angular offset from screen center to the crosshair, build a "look at"
    -- CFrame, then rotate it so the target shifts off-center to land under
    -- the crosshair. This makes the aim work correctly in games whose
    -- in-built crosshair is not at the screen center.
    -- When Cursor Mode is OFF: classic behaviour — target lands at center.
    local camPos = Camera.CFrame.Position
    local lookCF = CFrame.lookAt(camPos, aimPos)
    local targetCF
    if S.aimlockCursorMode then
        local vp     = Camera.ViewportSize
        ...
        targetCF = lookCF * CFrame.Angles(pitchOff, yawOff, 0)
    else
        targetCF = lookCF
    end
...
studsBtnObj.frame.BackgroundColor3    = Color3.fromRGB(35, 35, 45)
studsBtnObj.frame.BackgroundTransparency = 0.25
studsBtnObj.frame.Visible             = true
do
    local r = Instance.new("UICorner")
    r.CornerRadius = UDim.new(0.18, 0); r.Parent = studsBtnObj.frame
end
...
local function buildUI()
    ...
    AT:CreateSection("Master  (works locally — PC keybind [P], mobile uses on-screen button)")
    ...
    AT:CreateToggle({ Name = "Lock Player (sticky — keep target until aimlock off / target dies)",
        CurrentValue = S.aimlockStickyLock, Flag = "AimSticky",
        Callback = function(v) S.aimlockStickyLock = v end })
    AT:CreateToggle({ Name = "Smart AI (lead moving targets — predicts position)",
    ...
    AT:CreateSlider({ Name = "Max Camera Turn / Frame (0 = no limit)", Range = {0, 90}, Increment = 1, Suffix = "°",
        CurrentValue = S.aimlockMaxDegPerFrame, Flag = "AimMaxDeg",
        Callback = function(v) S.aimlockMaxDegPerFrame = v end })
    AT:CreateSlider({ Name = "Reacquire Delay (sticky lock grace)", Range = {0, 30}, Increment = 1, Suffix = " ×0.1s",
    ...
    AT:CreateInput({ Name = "Target Body Part (blank = use priority chain Head→Torso→HRP)",
    ...
    AT:CreateToggle({ Name = "Cursor Mode (target lands at the on-screen crosshair pixel, not screen-center)",
        CurrentValue = S.aimlockCursorMode, Flag = "AimCursorMode",
    ...
    AT:CreateToggle({ Name = "Predict Acceleration (use a²t² lead in addition to velocity)",
    ...
    AT:CreateSlider({ Name = "Miss Chance (skip aiming on N% of frames — looks human)",
    ...
    lpDD = AT:CreateDropdown({
        Name = "Lock Player List (tap to toggle — multi-select)",
        Options = _serverList(),
